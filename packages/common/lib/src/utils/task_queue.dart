...
class _QueuedFuture<T> {
  ...
  _QueuedFuture(this.closure, this.completer, this.timeout, {this.onComplete, this.key});
...
class TaskQueue {
  ...
  /// Whether auto process task as flow.
  bool autoPorcess;
  ...
  bool get isEmpty => _nextCycle.isEmpty;
  ...
  TaskQueue({this.delay, this.parallel = 1, this.timeout, this.autoPorcess = true});
  ...
  Future<T?> add<T>(Future<T> Function() closure, {String? key}) {
    if (isCancelled) return Future.value(null);
    if(key != null && _keys.contains(key)) Future.value(null);

    _keys.add(key);

    final completer = Completer<T>();
    _nextCycle.add(_QueuedFuture<T>(closure, completer, timeout, key: key));
