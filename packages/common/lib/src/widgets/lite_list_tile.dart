...
class _RenderListTile extends RenderBox with SlottedContainerRenderObjectMixin<_ListTileSlot> {
  ...
  RenderBox? get background => childForSlot(_ListTileSlot.background);
  ...
  void performLayout() {
    ...
    //debugPrint('tileWidth: $tileWidth, leadingSize: $leadingSize, trailingSize: $trailingSize');
    assert(
    ...
    if (!hasSubtitle) {
      ...
      titleY = (tileHeight - middleHeight + minVerticalPadding * 2) / 2.0;
      subtitleY = titleY + titleSize.height + verticalTitlePadding;
    }

    final backgroundSize = _layoutBox(background,
        BoxConstraints.tightFor(width: tileWidth + contentPadding.horizontal, height: tileHeight + contentPadding.vertical));
    if (background != null) {
      _positionBox(background!, Offset(-contentPadding.left, -contentPadding.top));
    }
    ...
    //    to the top.
    double leadingY = tileHeight / 2.0 - leadingSize.height / 2.0;
    final double trailingY = leadingY +  (leadingSize.height - trailingSize.height) / 2.0;
